...
# of length 4.  Consider f(1) for example.  Look at
# butterfly_puzzle.svg; f(1) refers to square "1".  Then f(1) returns
# c(61,186, 90,165).  These are the leftmost numbers of the four sides
# in clockwise order (N,E,S,W) ["leftmost" means that you are looking
# at the piece oriented so the edge in question is uppermost].  A "NA"
...
a <- read.table("butterfly2.txt" , header=TRUE)
...
DLlist <- list()
upto <- 25  #use smaller values for debugging
...
{  #do first line
  jj <- c(
          outer(c("piece_whit","piece_yell","piece_blue","piece_brow","piece_oran"),1:5,paste,sep=""),
          paste("loc_",sprintf("%02i",1:25),sep=""),
          outer(c("whit_row","yell_row","blue_row","brow_row","oran_row"),1:5,paste,sep=""),
          outer(c("whit_col","yell_col","blue_col","brow_col","oran_col"),1:5,paste,sep=""),
          26:225
          )
