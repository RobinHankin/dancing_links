...
# Function alltabs() takes an integer n representing a place in the
...
alltabs <- function(n,side_types,description){
  ...
  names(out) <- paste(description,c("twelveclock",
                                    "_threeclock",
                                    "___sixclock",
                                    "__nineclock"),sep="_")
  return(out)
...
get_piece <- function(i){
  ...
  list(description=paste("location_L",i_string,"_piece_",substr(a[i,1],1,2),a[i,2],sep=""),
       side_types=a[i,3:6]
       )
}
...
for(i in 1:upto){#'i' cycles through the 25 pieces
  jj <- get_piece(i)
  for(j in 1:upto){ # 'j' cycles through the 25 positions
    DLlist <- c(DLlist, alltabs(j,jj$side_types, jj$description))
  }
}
...
write(NULL,file="data.txt",append=FALSE)
for(i in 1:length(DLlist)){

  where_tabs <- DLlist[[i]]
  u <- names(DLlist)[i]
  colour <- substr(u,20,21)
  piece  <- substr(u,20,22)

  loc <- as.numeric(substr(u,11,12))
  ...
  s <- paste( 
             sprintf("%02i",loc),
             piece, colour_row, colour_col, paste(where_tabs,collapse=" "),sep=" ")
