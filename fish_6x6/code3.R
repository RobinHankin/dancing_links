...
# Then a list called DLlist is created of length 36*36*4=5184, and
...
# big numbers in fish_6x6.svg, and returns an integer vector
# of length 4.  Consider f(1) for example.  This returns c(147,327,181,301).
...
#butterfly_puzzle.svg, with NULL meaning 'nothing'.
...
# Function getnums() takes a number n, 1<=n<=26 representing a place
...
# Now translate the names appearing in tiles_fish6x6.txt to numbers:

f <- function(x){
  levels(x) <- c('bluef', 'goldf', 'green','johnd','strip','blugo','gold2')
  c(x)
}

  a[,3] <- +f(a[,3]) # North side of a piece
  a[,4] <- +f(a[,4]) # East  side of a piece
  a[,5] <- -f(a[,5]) # South side of a piece
  a[,6] <- -f(a[,6]) # West  side of a piece
...
{  #do first line
  jj <- c(
          ...
          paste("loc_",sprintf("%02i",1:36),sep=""),
          outer(c("puffa_row","zebra_row","angel_row","nemo__row","color_row","coral_row"),1:6,paste,sep=""),
          outer(c("puffa_col","zebra_col","angel_col","nemo__col","color_col","coral_col"),1:6,paste,sep=""),
          37:336   # small numbers on fish_6x6.svg
          )
...
for(i in seq_along(DLlist)){
  ...
  colour <- substr(u,7,11) # note difference from butterfly/code2.R (due to longer fish type names)
  piece  <- paste("piece_",substr(u,7,12),sep="")  # note difference from butterfly

  loc <- as.numeric(substr(u,23,24))  # note difference from butterfly
